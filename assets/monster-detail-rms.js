(() => {
  ...
  function sprite(monster) {
    ...
    return `<a class="rz-monster-sprite" href="#/monsters/${encodeURIComponent(monster.id)}"><img src="${MOB_IMAGE_BASE}${id}.png" alt="${esc(monster.name)}" loading="lazy" decoding="async" onerror="this.style.display='none';this.nextElementSibling.style.display='inline'" /><span style="display:none">${NA}</span></a>`;
  }
  ...
  style.textContent = `
    ...
    .rz-monster-db-results{display:grid;gap:20px;margin-top:10px}.rz-monster-db-results .rz-monster-sheet-wrap{margin:0}
