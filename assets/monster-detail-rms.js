(() => {
  ...
  const sourceTip=meta=>Object.entries(meta?.sources||{}).map(([k,v])=>`${k}: ${v===null||v===undefined?'unknown':v}`).join(' · ');
        ...
        <tbody>
          <tr>
            <td colspan="2" rowspan="2" class="rz-monster-left-block"><table class="rz-monster-subtable"><tbody>
              <tr><th>HP</th><td>${stat(monster,'hp',monster.hp)}</td></tr><tr><th>Level</th><td>${val(monster.level)}</td></tr><tr><th>Race</th><td>${val(monster.race)}</td></tr><tr><th>Property</th><td>${property}</td></tr><tr><th>Size</th><td>${val(monster.size)}</td></tr><tr><th>Hit (100%)</th><td>${stat(monster,'hit',monster.hit)}</td></tr><tr><th>Flee (95%)</th><td>${stat(monster,'flee',monster.flee)}</td></tr><tr><th>Walk Speed</th><td>${val(monster.walkSpeed)}</td></tr><tr><th>Atk Delay</th><td>${val(monster.attackDelay)}</td></tr><tr><th>Attack</th><td>${attack}</td></tr><tr><th>Magic Atk</th><td>${magicAttack}</td></tr><tr><th>Def</th><td>${stat(monster,'def',monster.def)}</td></tr><tr><th>Magic Def</th><td>${stat(monster,'mdef',monster.mdef)}</td></tr><tr><th>Atk Range</th><td>${stat(monster,'attackRange',monster.attackRange)}</td></tr><tr><th>Spell Range</th><td>${val(monster.spellRange)}</td></tr><tr><th>Sight Range</th><td>${val(monster.sightRange)}</td></tr>
            </tbody></table></td>
          ...
          <tr>
            <td colspan="2" class="rz-monster-midstats"><table class="rz-monster-subtable"><tbody>
              <tr><th>Base Exp</th><td>${stat(monster,'baseExp',monster.baseExp)}</td></tr><tr><th>Job Exp</th><td>${stat(monster,'jobExp',monster.jobExp)}</td></tr><tr><th>Base Exp / HP</th><td>${ratio(monster.baseExp,monster.hp)}</td></tr><tr><th>Job Exp / HP</th><td>${ratio(monster.jobExp,monster.hp)}</td></tr><tr><th>Delay After Hit</th><td>${val(monster.delayAfterHit)}</td></tr>
            </tbody></table><table class="rz-monster-subtable rz-monster-attributes"><tbody>
              <tr><th>Str</th><td>${stat(monster,'str',monster.str)}</td><th>Int</th><td>${stat(monster,'int',monster.int)}</td></tr><tr><th>Agi</th><td>${stat(monster,'agi',monster.agi)}</td><th>Dex</th><td>${stat(monster,'dex',monster.dex)}</td></tr><tr><th>Vit</th><td>${stat(monster,'vit',monster.vit)}</td><th>Luk</th><td>${stat(monster,'luk',monster.luk)}</td></tr>
            </tbody></table></td>
