(() => {
  ...
  const MOB_IMAGE_BASE = 'https://static.divine-pride.net/images/mobs/png/';
  ...
  function monsterFromHash() {
    ...
    if (!m || !window.RO_DATA || !Array.isArray(window.RO_DATA.monsters)) return null;
  ...
  function modeRows(monster) {
    ...
    return rows.map(x => `<div class="rz-monster-mode-row">- ${esc(x)}</div>`).join('');
  }
  ...
  function elementRows(monster) {
    ...
    return elements.map(name => `<tr><th>${name}</th><td>${Object.prototype.hasOwnProperty.call(modifiers,name) ? `${esc(modifiers[name])}%` : NA}</td></tr>`).join('');
  }
  ...
  function sprite(monster) {
    const id = num(monster.clientId || monster.id);
    if (id === null) return `<div class="rz-monster-sprite-empty">${NA}</div>`;
    return `<div class="rz-monster-sprite"><img src="${MOB_IMAGE_BASE}${id}.png" alt="${esc(monster.name)}" loading="eager" decoding="async" onerror="this.style.display='none';this.nextElementSibling.style.display='inline'" /><span style="display:none">${NA}</span></div>`;
  }
  ...
  function renderSheet(monster) {
    const property = monster.element && monster.element !== 'n/a'
      ? `${esc(monster.element)}${monster.elementLevel ? ` ${esc(monster.elementLevel)}` : ''}`
      : NA;
    ...
    return `<div class="rz-monster-sheet-wrap">
      <table class="rz-monster-sheet">
        <thead><tr><th colspan="8" class="rz-monster-title">${esc(monster.name)} <span>(${internal})</span> <span>Mob-ID#${id}</span></th></tr></thead>
        <tbody>
          <tr>
            <td colspan="2" rowspan="2" class="rz-monster-left-block">
              <table class="rz-monster-subtable"><tbody>
                <tr><th>HP</th><td>${val(monster.hp)}</td></tr>
                <tr><th>Level</th><td>${val(monster.level)}</td></tr>
                <tr><th>Race</th><td>${val(monster.race)}</td></tr>
                <tr><th>Property</th><td>${property}</td></tr>
                <tr><th>Size</th><td>${val(monster.size)}</td></tr>
                <tr><th>Hit (100%)</th><td>${val(monster.hit)}</td></tr>
                <tr><th>Flee (95%)</th><td>${val(monster.flee)}</td></tr>
                <tr><th>Walk Speed</th><td>${val(monster.walkSpeed)}</td></tr>
                <tr><th>Atk Delay</th><td>${val(monster.attackDelay)}</td></tr>
                <tr><th>Attack</th><td>${attack}</td></tr>
                <tr><th>Def</th><td>${val(monster.def)}</td></tr>
                <tr><th>Magic Def</th><td>${val(monster.mdef)}</td></tr>
                <tr><th>Atk Range</th><td>${val(monster.attackRange)}</td></tr>
                <tr><th>Spell Range</th><td>${val(monster.spellRange)}</td></tr>
                <tr><th>Sight Range</th><td>${val(monster.sightRange)}</td></tr>
              </tbody></table>
            </td>
            <td colspan="2" class="rz-monster-sprite-cell">${sprite(monster)}</td>
            <td colspan="2" rowspan="1" class="rz-monster-maps"><div class="rz-monster-section-title">On Maps</div>${mapRows(monster)}</td>
          ...
          <tr>
            <td colspan="2" class="rz-monster-midstats">
              <table class="rz-monster-subtable"><tbody>
                <tr><th>Base Exp</th><td>${val(monster.baseExp)}</td></tr>
                <tr><th>Job Exp</th><td>${val(monster.jobExp)}</td></tr>
                ...
                <tr><th>Delay After Hit</th><td>${val(monster.delayAfterHit)}</td></tr>
              </tbody></table>
              <table class="rz-monster-subtable rz-monster-attributes"><tbody>
                <tr><th>Str</th><td>${val(monster.str)}</td><th>Int</th><td>${val(monster.int)}</td></tr>
                <tr><th>Agi</th><td>${val(monster.agi)}</td><th>Dex</th><td>${val(monster.dex)}</td></tr>
                <tr><th>Vit</th><td>${val(monster.vit)}</td><th>Luk</th><td>${val(monster.luk)}</td></tr>
              </tbody></table>
            </td>
            <td colspan="2" class="rz-monster-mode"><div class="rz-monster-section-title">Mode</div>${modeRows(monster)}</td>
          </tr>
          <tr><th colspan="8" class="rz-monster-wide-title">Drops</th></tr>
          <tr><td colspan="8" class="rz-monster-wide-body">${dropRows(monster)}</td></tr>
          <tr><th colspan="8" class="rz-monster-wide-title">Monster Skills</th></tr>
          <tr><td colspan="8" class="rz-monster-wide-body">${skillRows(monster)}</td></tr>
        </tbody>
      </table>
    </div>`;
  }
